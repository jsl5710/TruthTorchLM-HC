...
\disaad~\citep{cui2026disaad} adds an adversarial term to sharpen the surrogate's
distribution. A natural third variant, which we call \ours, adds an
...
\[
\begin{aligned}
\dald  &: \; \mathcal{L}_{\text{SFT}}^{\text{masked}} \quad(\text{response tokens only})\\
\disaad&: \; \mathcal{L}_{\text{SFT}} + \mathcal{L}_{\text{adv}}\\
\ours  &: \; \mathcal{L}_{\text{SFT}} + \mathcal{L}_{\text{adv}} + \lambda\,\mathcal{L}_{\text{unc}}
\end{aligned}
\]
...
\section{Full Per-Read-Out Results}\label{app:readouts}
Table~\ref{tab:app-readout} gives the complete nine-read-out $\times$ seven-dataset AUROC
