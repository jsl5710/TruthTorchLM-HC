...
that (a) the best method is strongly \emph{generator-dependent} (graph-spectral and
P(True) lead on Qwen; verbalized confidence leads on Llama at 0.707 AUROC); (b)
...
\url{https://github.com/jsl5710/TruthTorchLM-HC}}
...
counted as a pure-BB result. Every consistency method is swept over sample budgets
...
Verbalized conf.\ ($N{=}1$)       & \best{0.728} & 0.500 & \best{0.702} & 0.546 & 0.668 & \best{0.686} & 0.483 & 0.616\\
Eccentricity ($N{=}10$)           & 0.817 & 0.500 & 0.522 & \best{0.614} & 0.764 & 0.607 & 0.564 & 0.627\\
Semantic entropy ($N{=}10$)       & \best{0.819} & 0.500 & 0.500 & 0.623 & 0.710 & 0.557 & 0.533 & 0.606\\
Lexical similarity ($N{=}10$)     & 0.695 & 0.500 & 0.539 & 0.580 & 0.575 & 0.501 & \best{0.607} & 0.571\\
Distilled proxy ($N{=}0$)         & 0.618 & --    & 0.705 & 0.523 & 0.780 & 0.472 & 0.593 & 0.612\\
\grey{P(True) (grey-box)}         & \grey{0.827} & \grey{0.500} & \grey{0.690} & \grey{0.595} & \grey{\best{0.838}} & \grey{0.623} & \grey{0.560} & \grey{\best{0.662}}\\
\midrule
\multicolumn{9}{l}{\emph{Target: Llama-3.3-70B}}\\
Verbalized conf.\ ($N{=}1$)       & 0.656 & 0.966 & \best{0.699} & \best{0.648} & 0.627 & \best{0.797} & \best{0.557} & \best{0.707}\\
Eccentricity ($N{=}10$)           & \best{0.796} & 0.973 & 0.585 & 0.653 & 0.702 & 0.500 & 0.526 & 0.677\\
Semantic entropy ($N{=}10$)       & 0.630 & 0.487 & 0.496 & 0.501 & 0.585 & 0.473 & 0.492 & 0.523\\
Lexical similarity ($N{=}10$)     & 0.758 & \best{1.000} & 0.667 & 0.456 & 0.591 & 0.518 & 0.544 & 0.648\\
Distilled proxy ($N{=}0$)         & 0.631 & 0.993 & 0.600 & 0.450 & 0.694 & 0.551 & 0.507 & 0.632\\
\grey{P(True) (grey-box)}         & \grey{0.690} & \grey{1.000} & \grey{0.549} & \grey{0.567} & \grey{0.711} & \grey{0.564} & \grey{0.535} & \grey{0.659}\\
\bottomrule
\end{tabular}
...
ranking. Bioasq is degenerate on Qwen (trivially separable) and omitted from its mean.}
\label{tab:leaderboard}
\end{table*}

\paragraph{The best method is generator-dependent.} On Qwen3-32B the pure-BB winners are
graph-spectral (Eccentricity 0.627) with verbalized confidence mid-pack (0.616); on
Llama-3.3-70B \emph{verbalized confidence dominates} (0.707), far ahead of the same
graph-spectral method (0.677) and with semantic entropy \emph{collapsing} to near-chance
(0.523). The closed targets sharpen this (Appendix~\ref{app:closed}): verbalized
...
Eccentricity      & 0.500 & 0.594 & 0.620 & \best{0.627}\\
SumEigen          & 0.500 & 0.596 & 0.618 & \best{0.634}\\
Semantic entropy  & 0.500 & 0.560 & 0.588 & \best{0.606}\\
Lexical sim.      & 0.500 & 0.555 & 0.560 & \best{0.571}\\
Verbalized conf.  & \best{0.616} & 0.616 & 0.616 & 0.616\\
\midrule
\multicolumn{5}{l}{\emph{Llama-3.3-70B}}\\
Eccentricity      & 0.500 & 0.642 & 0.671 & \best{0.677}\\
Lexical sim.      & 0.500 & 0.643 & 0.645 & \best{0.648}\\
Semantic entropy  & 0.500 & 0.513 & 0.517 & \best{0.523}\\
Verbalized conf.  & \best{0.707} & 0.707 & 0.707 & 0.707\\
\bottomrule
\end{tabular}
...
\paragraph{Sample budget is a hidden cost.} Table~\ref{tab:nsweep} makes the
...
Verbalized conf.        & 0.616 & 0.898 & 0.280\\
Eccentricity ($N{=}10$) & \best{0.627} & 0.840 & 0.140\\
Semantic entropy ($N{=}10$) & 0.606 & \best{0.913} & 0.283\\
Lexical sim.\ ($N{=}10$)& 0.571 & 0.842 & 0.067\\
Distilled proxy (native)& 0.560 & 0.819 & 0.155\\
\grey{P(True) (grey)}   & \grey{0.662} & \grey{0.890} & \grey{\best{0.391}}\\
\midrule
\multicolumn{4}{l}{\emph{Llama-3.3-70B}}\\
Verbalized conf.        & \best{0.707} & \best{0.915} & 0.352\\
Eccentricity ($N{=}10$) & 0.677 & 0.859 & \best{0.402}\\
Semantic entropy ($N{=}10$) & 0.523 & 0.902 & 0.112\\
Lexical sim.\ ($N{=}10$)& 0.648 & 0.866 & 0.342\\
Distilled proxy (native)& 0.607 & 0.851 & 0.307\\
\grey{P(True) (grey)}   & \grey{0.659} & \grey{0.909} & \grey{0.350}\\
\bottomrule
\end{tabular}
\caption{Beyond AUROC: three scale-robust ranking metrics (mean over datasets). AUPRC is
uniformly high (correct-answer base rate); PRR discriminates where AUPRC cannot. The full
eight-metric suite (adding AUARC, ECE, ACE, MCE, Brier) is in the release. Proxy shown at
its native read-out; grey rows are grey-box.}
\label{tab:metrics}
\end{table}
...
regeneration scripts are released at \url{https://github.com/jsl5710/TruthTorchLM-HC}; the
103 distilled proxy adapters are on the Hugging Face Hub at
\url{https://huggingface.co/jsl5710/TruthTorchLM-HC-proxies}.
...
makes the generator-dependence unmistakable: the Qwen leaderboard is topped by
graph-spectral scores and the Llama leaderboard by verbalized confidence, with semantic
entropy strong on Qwen (0.606) but near-chance on Llama (0.523).
...
\midrule
Verbalized confidence         & 0.616 & \best{0.707}\\
Eccentricity (Unc, $N{=}10$)  & 0.627 & 0.677\\
Eccentricity (Conf, $N{=}10$) & 0.633 & 0.668\\
SumEigen / EigV ($N{=}10$)    & \best{0.634} & 0.639\\
MatrixDegree (Unc, $N{=}10$)  & \best{0.634} & 0.639\\
MatrixDegree (Conf, $N{=}10$) & 0.629 & 0.645\\
Lexical similarity ($N{=}10$) & 0.571 & 0.648\\
Kernel language entropy ($N{=}10$) & 0.604 & 0.619\\
Discrete semantic entropy ($N{=}10$) & 0.606 & 0.523\\
Num.\ semantic sets ($N{=}10$)& 0.605 & 0.523\\
Distilled proxy ($N{=}0$)     & 0.612 & 0.632\\
\grey{P(True) (grey-box)}     & \grey{\best{0.662}} & \grey{0.672}\\
\bottomrule
\end{tabular}
\caption{Full pure-BB leaderboard, two open targets, mean AUROC at each method's best
sample budget (48 configs scored per target). Grey-box P(True) shown for reference.}
\label{tab:app-open}
\end{table}
...
dispersion of its (tightly-decoded, self-consistent) samples. Two honest caveats: on
...
and lexical labels are retained and the judge is recorded per cell.
