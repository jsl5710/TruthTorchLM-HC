...
actionable: \textbf{the read-out dominates the training objective}. Holding the proxy
fixed, the choice of logit read-out moves AUROC by $0.107$--$0.109$, while the three training
objectives---at any fixed read-out---span only $0.011$--$0.028$, a $4$--$10\times$ gap; we
therefore find no evidence that the objectives differ at deployable operating points. Which
read-out is best is itself teacher-dependent (perplexity on one teacher, evidential on the
other), so the actionable advice concerns the read-out axis, not a specific estimator. We further show,
...
\url{[repository URL withheld for anonymous review]}. The distilled proxy adapters (103 LoRA proxies) are on the Hugging Face Hub: \url{[model-hub URL withheld for anonymous review]}. Every $(G,D)$ target generation is cached
