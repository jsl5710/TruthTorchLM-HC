...
conditions: papers assume white-box logits or black-box text, use one target and a handful of
datasets, and---almost universally---ignore the \emph{cost} of producing the signal. We present a
benchmark organized around four explicit axes, $G$ (target), $D$ (dataset), $M$ (method), and
$V$ (metric), with two commitments existing suites lack: \textbf{pure black-box} semantics
enforced at the interface---methods see \emph{text} only, and anything reading target
log-probabilities (e.g.\ P(True)) is labelled grey-box and reported separately---and
\textbf{latency measured relative to the target}, making the accuracy--cost trade a first-class
result. We instantiate it with four frontier targets (Qwen3-32B, Llama-3.3-70B, GPT-4o,
Claude-Haiku-4.5), seven domains, and $75$ method configurations spanning sampling-consistency,
graph-spectral, verbalized, and distilled-proxy families, swept over budgets
$N\in\{1,3,5,10\}$. A cached, content-hashed generation per $(G,D)$ guarantees identical inputs.
Across ${\approx}1{,}700$ evaluations we find (a) the best method is \emph{generator-dependent}
(graph-spectral and verbalized confidence tie on Qwen at ${\approx}0.65$; verbalized leads on
Llama at $0.664$, where semantic entropy falls to near-chance); (b) consistency methods need
$N{=}10$ target generations to peak, a cost invisible in fixed-$N$ tables; and (c) distilled
proxies reach competitive AUROC at a target-decoupled $24$--$44$\,ms. We release the harness,
...
$\times$ $75$ method configurations (sampling-consistency, graph-spectral, verbalized,
...
graph-spectral \citep{lin2024generating} methods, or verbalized confidence
...
zero target calls \emph{at inference} (one small student forward); consistency methods pay $N$;
verbalized/P(True) pay one. The proxy's zero is an inference-time figure and excludes a
one-off training cost: $400$ distillation prompts $\times$ $10$ target samples $=$ $4{,}000$
target generations, plus LoRA fine-tuning. Amortized, the proxy breaks even against $N{=}10$
consistency after ${\approx}400$ scored queries and against one-call verbalized confidence after
${\approx}4{,}000$; past that its marginal target cost is zero while theirs is not. $75$ total configurations: $12$ families $\times$ $4$ budgets, plus $27$ proxy read-out configurations.}
\label{tab:roster}
\end{table}

\paragraph{What each family computes.} So that Table~\ref{tab:leaderboard} can be read without
the appendix: \emph{semantic-set} methods cluster the $N$ samples into meaning-equivalent groups
with an entailment model and score the entropy of the cluster distribution (Discrete Semantic
Entropy; NumSemanticSet is the raw cluster count). \emph{Graph-spectral} methods build a
similarity graph over the samples and read its Laplacian spectrum---EigV counts near-zero
eigenvalues (how many distinct semantic components exist), Eccentricity measures how far the
samples sit from their common centroid in the spectral embedding, and MatrixDegree/SumEigen are
degree- and eigenvalue-sum summaries of the same graph. \emph{Lexical similarity} replaces the
entailment graph with mean pairwise ROUGE-L. All of these score \emph{disagreement among
resamples}, which is why they are exactly $0.5$ at $N{=}1$. \emph{Verbalized confidence} asks the
target to rate its own answer once. \emph{Distilled proxies} run a small student trained on the
target's text and read a scalar off its logits---either the distribution's shape (max-softmax,
entropy, energy, evidential AU/EU, LogTokU) or the likelihood it assigns to the tokens the target
actually emitted (perplexity, MaxNLL, LogitMargin); ``AU/EU'' are the aleatoric and epistemic
terms of a Dirichlet fit to the top-$K$ logits. \emph{P(True)} asks the target for the probability
of the token ``true'' and therefore needs log-probabilities, which is why we label it grey-box.
Exact formulas for every method, and the three proxy training objectives, are in
...
sample budget. \textbf{This is an oracle operating point, not a deployable one}: $N$ (and, for
the proxy, the read-out) is chosen post hoc on the same $150$ items the AUROC is computed on,
with no held-out split. The selection is not symmetric across families---the proxy is chosen
from $3$ objectives $\times$ $9$ read-outs while verbalized confidence has nothing to select---so
the proxy row is the most optimistic of the table. We quantify the size of that bias in
\S\ref{sec:results} and report the unselected native read-out alongside it in
Table~\ref{tab:metrics}; the ranking claims we make are the ones that survive it. Two cross-axis phenomena stand out.
...
degenerate-cell exclusion, so we compare methods \emph{within} a target, never across. The full per-target leaderboard is in Appendix~\ref{app:leaderboard}. The closed targets
sharpen this (Appendix~\ref{app:closed}): verbalized
...
(Table~\ref{tab:latency}, Fig.~\ref{fig:pareto2}). The three families occupy distinct regimes. Distilled proxies
...
\begin{tabular}{l c c}
\toprule
method class & target calls & scoring latency / item\\
\midrule
Distilled proxy   & $0$  & \textbf{24--44 ms}\\
\grey{P(True) (grey)} & \grey{$1$} & \grey{109--205 ms}\\
Verbalized conf.  & $1$  & 286--287 ms\\
Semantic entropy ($N{=}10$) & $10$ & 541--551 ms\\
Lexical sim.\ ($N{=}10$)    & $10$ & 1275--1336 ms\\
Eccentricity ($N{=}10$)     & $10$ & 3133--3239 ms\\
\bottomrule
\end{tabular}
\caption{Latency frontier by method class, all values \emph{measured} (p50 over
non-degenerate cells, both open targets; batch 1, GPU warm, exclusive node). These are
\textbf{scoring} costs and exclude the target calls in column two, which dominate: a method with
$N{=}10$ additionally pays ten target generations, seconds each on a 70B and $\approx$1--2\,s on
an API. Only the proxy's figure is complete as shown, because it makes no target call. On
scoring compute alone the proxy is $71$--$135\times$ cheaper than $N{=}10$ Eccentricity; once the
ten API generations are included the end-to-end ratio is $250$--$790\times$
(\S\ref{sec:latency}).}
\label{tab:latency}
\end{table}
...
\begin{axis}[
  ...
  xlabel={measured scoring latency per item (ms, log scale)},
  ylabel={mean AUROC},
  xmin=15, xmax=6000, ymin=0.50, ymax=0.72,
  xlabel style={font=\small}, ylabel style={font=\small},
  tick label style={font=\scriptsize},
  grid=major, grid style={gray!22}, axis line style={gray!60},
...
\addlegendentry{distilled proxy ($0$ calls)}
\addplot[only marks, mark=triangle*, mark size=3pt, gray!55!black] coordinates {(286,0.664)(287,0.653)};
\addlegendentry{verbalized ($1$ call)}
\addplot[only marks, mark=*, mark size=2.4pt, gray!75!black]
  coordinates {(551,0.529)(541,0.624)(1336,0.589)(1275,0.583)(3239,0.627)(3133,0.655)};
\addlegendentry{consistency / spectral ($10$ calls)}
\end{axis}
\end{tikzpicture}
\caption{\textbf{Accuracy against measured scoring latency}, two open targets (two points per
class). The $x$ axis is the method's own compute only; the $10$-call family additionally pays ten
target generations, which moves it one to two further decades to the right and is the cost this
benchmark exists to expose. Note the frontier is not a clean trade: the cheapest family (proxies)
is not the least accurate, and the most expensive scoring method (Eccentricity) does not lead on
either target.}
\label{fig:pareto2}
\end{figure}
...
\section*{Limitations}
The closed-target arm is not reproducible in the way the open arm is. Our gateway exposes GPT-4o
and Claude-Haiku-4.5 as version aliases rather than pinned snapshots, and companion measurements
show that re-scoring the same cached generations two days apart can move greedy comparisons by
more than $0.10$ AUROC (Appendix~\ref{app:closed}). Closed-target conclusions here should be
read as holding for the snapshot that answered on the run date, not for ``GPT-4o'' as a stable
object.
% ==============================================================================
...
relation $E(\cdot,\cdot)$~\citep{kuhn2023semantic,farquhar2024detecting}; let $\mathcal{C}=
...
(y^{(a)},y^{(b)})$ (NLI-entailment or Jaccard), degree matrix $\mathbf{\Delta}$ (we write
$\mathbf{\Delta}$, not $D$, since $D$ is the dataset axis), and normalized Laplacian
$L = I - \mathbf{\Delta}^{-1/2} W \mathbf{\Delta}^{-1/2}$ with eigenvalues $\lambda_1\le\dots\le\lambda_N$
...
about their centroid; \emph{MatrixDegree} uses the normalized total degree $1-\tfrac{1}{N^{2}}\sum_a \mathbf{\Delta}_{aa}$; \emph{SumEigen} sums $\lambda_k$. Confidence/uncertainty variants negate the
...
$\mathcal{D}_{\mathrm{distill}}=\{(x^{(i)},y_B^{(i,j)})\}_{i,j=1}^{P,Q}$ ($P$ prompts $\times$ $Q$
teacher samples; we use $P,Q$ rather than $N,M$, which denote the sample budget and the method
axis elsewhere in this paper); they differ only in the loss.
\textbf{DALD}~\citep{zeng2024dald} is masked distillation (response tokens only),
$\mathcal{L}_{\mathrm{task}}(\theta)=-\tfrac{1}{PQ}\sum_{i,j}\sum_t \log P_\theta\!\left(y^{(i,j)}_t\mid y^{(i,j)}_{<t},x^{(i)}\right)$. \textbf{DisAAD} (Distribution-Aligned Adversarial Distillation)
...
\mathcal{L}_{\mathrm{reg}}(\theta)=-\tfrac{1}{PQ}\!\sum_{i,j}\!\log \mathcal{M}_D\big(x^{(i)},\tilde{y}^{(i,j)};\phi\big),
\end{equation}
alternating with the discriminator objective $\mathcal{L}_D(\phi)=-\tfrac{1}{PQ}\sum_{i,j}
[\log \mathcal{M}_D(x^{(i)},y_B^{(i,j)};\phi)+\log(1-\mathcal{M}_D(x^{(i)},\tilde{y}^{(i,j)};\phi))]$,
so the proxy's own response $\tilde{y}$ becomes indistinguishable from the target's $y_B$.
Because $\tilde{y}$ is discrete text, the adversarial gradient reaches $\theta$ through the
proxy's output distribution rather than through sampled tokens. \textbf{Ours} keeps
...
\paragraph{A reproducibility caveat on this arm.} The gateway exposes these targets as version
\emph{aliases}, not pinned snapshots. In companion work using the same gateway and the same
cached generations, re-scoring the two closed targets two days apart moved $12$ of $32$
\emph{greedy} ($T{=}0$) comparisons by more than $0.10$ AUROC, with identical items, labels and
class balances and no failed calls---consistent with the alias being re-pointed to a newer
snapshot between runs. The closed numbers below are therefore a measurement of
\emph{whatever the alias resolved to on the run date}, and should be read with wider
uncertainty than the open-target numbers, which are fully reproducible locally. We recommend
that black-box benchmarks record the resolved snapshot id with every closed-target cell; ours
did not, and that is a limitation of this release.

One protocol constraint is permanent: closed APIs do not expose
...
gateway elicitation matching the open prompt. \textbf{Caveat:} this breaks the input-matching
guarantee that holds everywhere else in the benchmark---the verbalized score is elicited live,
whereas the consistency methods score the \emph{cached} generations---so the closed-target
comparison is not strictly input-matched and should be read as indicative. Re-running verbalized
confidence against the cached answers is the fix, and is supported by the released harness.
Table~\ref{tab:app-closed} tells a sharp
story: the \emph{sampling-consistency} families \textbf{degrade badly}---GPT-4o tops at
$\sim$0.63, and on \textbf{Claude-Haiku every consistency method is at or below $0.55$}
