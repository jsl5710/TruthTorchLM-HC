...
Llama-3.3-70B; closed GPT-4o and Claude-Haiku-4.5), seven task domains, and $75$
method configurations ($12$ method families $\times$ $4$ sample budgets, plus $27$ distilled-proxy
configurations at $N{=}0$) spanning sampling-consistency, graph-spectral, verbalized, and
...
identical inputs. Across ${\approx}1{,}700$ method$\times$dataset$\times$target evaluations (each
scored on eight metrics) we find
...
sampling-consistency methods are uninformative at $N{=}1$ by construction (a single sample has
no dispersion) and need $N{=}10$ samples---ten target generations---to reach their peak, which we
quantify as a cost curve rather than report as a finding; and (c) distilled proxies
reach competitive AUROC at a \emph{target-decoupled} $24$--$44$\,ms, hundreds of times
