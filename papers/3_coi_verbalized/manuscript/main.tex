...
Across six targets (8B to 70B open, plus closed GPT-4o and Claude-Haiku) and up to twelve
datasets, excluding cells too class-imbalanced to support an AUROC, the result is mostly
\emph{negative}: verification wins a minority of cells---$2$--$3$ of $11$ on 8B targets and
\textbf{none of the $9$} on the strong open 32B/70B targets. Under a paired bootstrap exactly
one positive effect reaches significance: on \textbf{long-form general-knowledge factuality}
(LongFact, $n{=}300$), self-consistency verification gains $+0.105$ AUROC over verbalized
confidence on Llama ($p{=}0.041$) and $+0.092$ on Qwen in the same direction but without
significance ($p{=}0.107$)---and this is the one regime where multi-sample baselines cannot be
applied at all. We also test whether verification rescues \emph{overconfident} targets whose
verbalized AUROC falls below chance: of the two inverted cells with adequate class balance, it
recovers one and fails the other, so we leave this open. Reporting confidence intervals
throughout, we find this design resolves only differences above $\sim$$0.15$ AUROC, and we
label the remaining comparisons inconclusive rather than null. The recommendation is
...
\item \textbf{We report our statistical power honestly.} A paired bootstrap over the cells with
...
every other comparison in the paper is inconclusive rather than null, and we label it as such.
...
\subsection{The exception: long-form general knowledge}
...
two-sided bootstrap $p$-value against $\Delta{=}0$.
