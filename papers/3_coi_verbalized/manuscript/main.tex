...
The reason is that imitative falsehoods and confident wrong options are produced \emph{
consistently}, so dispersion carries no signal. Where errors are inconsistent---general-knowledge
...
not a new estimator. We report per-cell bootstrap intervals, exclude cells too class-imbalanced
to support an AUROC, and release the harness and all per-item scores.
\end{abstract}
...
adversarial, math and multi-hop content, with every method scoring the same cached
generations.\footnote{Harness, per-item scores and analysis scripts:
...
\item \textbf{Which family wins is determined by the data, not the model.} Over $27$
...
estimator---and we report bootstrap intervals showing that many of the differences \emph{within}
the family are not resolvable at these sample sizes.
\end{enumerate}
...
Table~\ref{tab:family} is the paper's central result. Aggregated over $27$ non-degenerate cells
...
lose seven of eight general short-form cells and both general long-form cells.
...
\paragraph{Where sampling works, it earns its keep.} On general-knowledge short form the ordering
reverses---multi-sample methods win seven of eight cells, by a median $0.048$---and on LongFact
both. This is the regime the multi-sample literature was developed on, and our results reproduce
...
deterministic aggregation over self-rated risk ($n_3$)---take $23$ of $38$ cells between them,
